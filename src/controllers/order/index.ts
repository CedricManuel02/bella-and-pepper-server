...
import { cancelledOrderService, getAllOrdersService, getOrderItemService, getOrderService, getOrdersService, receivedOrderService } from "../../services/order/index.js";
...
export async function getOrderController(c: Context) {
    const {order_id, user_id} = await c.req.param();
...
export async function createOrderCancellationController(c: Context) {
    const {order_number, user_id} = await c.req.param();

    const body = await c.req.json();

    return c.json({message: "Successfully canceled order", status: StatusCodes.OK})
...
export async function receivedOrderController(c: Context) {
    ...
    return c.json({message: "Successfully updated order", status: StatusCodes.OK})
}
...
export async function cancelledOrderController(c: Context) {
    ...
    await cancelledOrderService({order_id: order_id, user_id: user_id, reason: body["reason"]});

    return c.json({message: "Successfully cancelled order", status: StatusCodes.ACCEPTED});
}