import { createCartData, deleteCartData, getCartByVariantIdData, getCartData, getCartsData, getExistingCartData, updateCartData } from "../../data/cart/index.js";
...
export async function deleteCartByVariantIdService(payload: { variant: any, user_id: string }) {
  ...
  for (let i = 0; i < payload.variant.length; i++) {
    ...
    if (!cart) {
      throw new BadRequestError(`Cart not found for variant_id: ${variant.variant_id}`);
    }
